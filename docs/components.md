...
| `dns` | network | passthrough | uncapped M/M/1; adds only the blended uncached-resolution latency; never the bottleneck | `routingPolicy`, `resolveLatencyMs`, `cacheHitRatio`, `ttlSec`, `healthCheckSec`, `capacityRps` |
